...
PRIMARY_SERIAL_CLS * debug_serial_port;
  ...
  uint8_t sspin = FEATURE_ETHERNET_SSPIN;     // Ethernet SS JL3OUW
...
static bool mode_wabun = false;      // JL3OUW
static bool tmp_eng = false;         // JL3OUW
  ...
  if (screen_refresh_status == SCREEN_REFRESH_IN_PROGRESS){
    if (x > lcd_scroll_buffer[y].length()){
      ...
      if (lcd_scroll_buffer[y].charAt(x) > 0){
          ...
          lcd.backlight();
        #endif  // FEATURE_LCD_BACKLIGHT_AUTO_DIM
          lcd.setCursor(0,y);    // wabun JL3OUW
          lcd.print(lcd_scroll_buffer[y]);   // wabun JL3OUW
          //lcd.print(lcd_scroll_buffer[y].charAt(x));   // wabun JL3OUW
      }
      x++;
    }
  }
...
void check_ps2_keyboard()
{
  ...
  #define PS2_HENKAN    162     // JL3OUW
  #define PS2_MUHENKAN  163     // JL3OUW
    ...
    if (ps2_keyboard_mode == PS2_KEYBOARD_NORMAL) {
      switch (keystroke) {
        ...
        case PS2_F6_CTRL :
          ...
          break;
        #endif //OPTION_SAVE_MEMORY_NANOKEYER
        #endif //OPTIONPS2_KEYBOARD_JAPANESE
        ...
        default :
          if ((keystroke > 31) && (keystroke < 255 /*123*/)) {
            ...
            if (keystroke == PS2_HENKAN) {    // JL3OUW
              mode_wabun = true;              // JL3OUW
            }                                 // JL3OUW
            if (keystroke == PS2_MUHENKAN) {  // JL3OUW
              mode_wabun = false;             // JL3OUW
            }                                 // JL3OUW
            if (keystroke == '[') {           // JL3OUW
              tmp_eng = true;                 // JL3OUW
            }                                 // JL3OUW
            if (keystroke == ']') {           // JL3OUW
              tmp_eng = false;                // JL3OUW
            }                                 // JL3OUW
...
void send_char(byte cw_char, byte omit_letterspace)
{
  ...
  if (char_send_mode == CW) {
    switch (cw_char) {
      ...
      case 93   : send_the_dits_and_dahs("-.--.-"); break;  // Start english in wabun_mode "KK" [
      case 91   : send_the_dits_and_dahs(".-..-."); break;  // Stop english in wabun_mode "RR" ]
      #endif
...
int convert_cw_number_to_ascii (long number_in) {
  ...
  switch (number_in) {
    ...
    //#ifdef OPTION_WABUN_CODE   JL3OUW
    ...
    // wabun JL3OUW start
      ...
      case 211222:                                           // Start wabun mode "DO(ﾎﾚ)" ｢
        mode_wabun = true;
        #ifdef FEATURE_PS2_KEYBOARD
           keyboard.SetKanaKeymap();
        #endif
        return 162; 
      break;

      case 11121:                                           // Stop wabun mode "SN(ﾗﾀ)" ｣
        mode_wabun = false;
        #ifdef FEATURE_PS2_KEYBOARD
           keyboard.SetJpKeymap();
        #endif
        return 163;
      break;

      case 212212:                                          // Start english in wabun_mode "KK" [
        tmp_eng = true;
        #ifdef FEATURE_PS2_KEYBOARD
           keyboard.SetEngTmpFlagOn();
        #endif
        return 91;
      break;

      case 121121:                                          // Stop english in wabun_mode "RR" ]
        tmp_eng = false;
        #ifdef FEATURE_PS2_KEYBOARD
           keyboard.SetEngTmpFlagOff();
        #endif
        return 93;
        break;
    #endif
...
void initialize_ethernet(){
  ...
  #if defined(FEATURE_ETHERNET)
    Ethernet.init(sspin);     // Ethernet CS  JL3OUW
